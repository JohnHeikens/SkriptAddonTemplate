package com.johnheikens.skriptaddontemplate.listeners;
...
import com.johnheikens.skriptaddontemplate.events.NamedSoundEvent;
import com.johnheikens.skriptaddontemplate.events.PlayerBlockDigEvent;
import com.johnheikens.skriptaddontemplate.events.SteerVehicleEvent;
import com.johnheikens.skriptaddontemplate.events.SteerVehicleEvent.Movement;
import com.johnheikens.skriptaddontemplate.wrappers.WrapperPlayClientBlockDig;
import com.johnheikens.skriptaddontemplate.wrappers.WrapperPlayClientUpdateSign;
import com.johnheikens.skriptaddontemplate.wrappers.WrapperPlayServerNamedSoundEffect;

public class PacketListeners {

	static {
		SkriptAddonTemplate instance = SkriptAddonTemplate.getInstance();
		...
		instance.getProtocolManager().addPacketListener(new PacketAdapter(instance, PacketType.Play.Client.BLOCK_DIG) {
			...
			public void onPacketReceiving(PacketEvent event) {
				...
				Bukkit.getScheduler().runTask(SkriptAddonTemplate.getInstance(), () -> Bukkit.getPluginManager().callEvent(dig));
				if (dig.isCancelled())
					event.setCancelled(true);
			}
		});
